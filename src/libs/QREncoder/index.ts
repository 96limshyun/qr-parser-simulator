...
import { QRMatrixBuilder } from "./QRMatrixBuilder";
...
export class QREncoder {
  ...
  convertToCodewords(bits: string): number[] {
    const out: number[] = [];
  ...
  createReedSolomonEncoder(): ReedSolomonEncoder {
    return new ReedSolomonEncoder(GenericGF.QR_CODE_FIELD_256);
  }

  prepareDataAndECCInfo(
    bitStream: string,
    version: number,
    errorCorrectionLevel: ErrorCorrectionLevel,
  ) {
    const dataCw = this.convertToCodewords(bitStream);
    ...
    return {
      dataCw,
      shardLen: eccInfo.dataCodewordsGroup1,
      eccLen: eccInfo.ecCodewordsPerBlock,
    };
  }

  generateECCCodewords(dataCw: number[], shardLen: number, eccLen: number): number[] {
    const encoder = this.createReedSolomonEncoder();
  ...
  combineFinalCodewords(dataCw: number[], eccCw: number[], shardLen: number): number[] {
    return [...dataCw.slice(0, shardLen), ...eccCw];
  }

  createFinalBitStream(finalCw: number[]): string {
    return finalCw.map((b) => b.toString(2).padStart(8, "0")).join("");
  }
  ...
  generateECC(
    ...
    const { dataCw, shardLen, eccLen } = this.prepareDataAndECCInfo(
      bitStream,
      version,
      errorCorrectionLevel,
    );
    const eccCw = this.generateECCCodewords(dataCw, shardLen, eccLen);
    const finalCw = this.combineFinalCodewords(dataCw, eccCw, shardLen);
    const finalBits = this.createFinalBitStream(finalCw);
  ...
  getBasePattern(
    bitStream: string,
    eccResult: { finalBits: string },
    errorCorrectionLevel: ErrorCorrectionLevel,
    smallestVersion: number,
  ) {
    const matrixBuilder = new QRMatrixBuilder(smallestVersion, errorCorrectionLevel);

    const finderPattern = matrixBuilder.findFinderPattern();
    const alignmentPattern = matrixBuilder.findAlignmentPattern();
    const timingPattern = matrixBuilder.findTimingPattern();
    const darkModule = matrixBuilder.findDarkModule();
    const dataModules = matrixBuilder.findDataModules(bitStream, eccResult.finalBits);

    return [...finderPattern, ...alignmentPattern, ...timingPattern, ...darkModule, ...dataModules];
  }

  getMaskedMatrixPositions(
    ...
    const matrixBuilder = new QRMatrixBuilder(smallestVersion, errorCorrectionLevel);
    const { maskedMatrixPositions, maskNumber, formatPosition } =
      matrixBuilder.applyMaskPattern(basePattern);

    return { maskedMatrixPositions, maskNumber, formatPosition };
  }
}
